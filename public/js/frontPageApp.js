...
let axios = __webpack_require__(14);
...
__WEBPACK_IMPORTED_MODULE_0_localforage___default.a.getItem('authtoken').then(authtoken => {
...
document.getElementById('hide-login-button').addEventListener("click", function () {
	hide('login-form');clearLoginForm();
});
document.getElementById('submit-login-button').addEventListener("click", function () {
	login();
});

document.getElementById('login-email').addEventListener("keydown", function () {
	clearNodeValue('login-root-error');
});
document.getElementById('login-password').addEventListener("keydown", function () {
	clearNodeValue('login-root-error');
});
...
document.getElementById('hide-register-button').addEventListener("click", function () {
	hide('register-form');clearRegistrationForm();
});
...
*/
	...
	axios.post('/api/login', { email: email.value, password: password.value } //add csrf field (by configuring axios!)
		...
		__WEBPACK_IMPORTED_MODULE_0_localforage___default.a.setItem('authtoken', response.data.meta.token).then(() => {
			window.location.replace(applicationUrl);
		});
	}).catch(error => {
...
const setHttpToken = token => {
	if (__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_1_lodash__["isEmpty"])(token)) {
